...
5. **Partially implemented:** Flow Layout 0.1 now has its public ABI, manifest,
   deterministic Layout Plan, and continuous, virtual-pages, and columns block
   slices, including cross-region text ranges and whole-object region
   advancement. The next gate is inline objects, float/overlay, and pagination
   controls.
