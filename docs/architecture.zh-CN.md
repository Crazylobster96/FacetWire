...
5. **部分实现：** Flow Layout 0.1 已落地公共 ABI、Manifest、确定性 Layout Plan、`continuous + block` 与 `virtual-pages + block`；已支持文本跨页连续范围和不可拆对象整体移页。下一门禁是 columns、inline object、float/overlay 和分页控制。
