...
void main() {
  ...
  testWidgets('verifies levels, fallback, virtual pages, and viewer mode', (
    ...
    expect(find.byKey(const ValueKey('flow-recursive-canvas')), findsOneWidget);
    expect(find.byKey(const ValueKey('flow-level-surface-0')), findsOneWidget);
    expect(find.byKey(const ValueKey('flow-level-surface-1')), findsOneWidget);
    expect(find.byKey(const ValueKey('flow-level-surface-2')), findsOneWidget);
    ...
    expect(
      find.byKey(const ValueKey('flow-fragment:object.missing.level-3:page-0')),
      findsOneWidget,
    );
