...
尚未支持：virtual pages、columns、inline object 实际排版、float、overlay、keep/widow/orphan、Playground 投影。相关合法请求返回 `FW_STATUS_UNSUPPORTED`。
...
The first experimental slice supports the public `facetwire.layout.flow.v1` C ABI, continuous block layout, Text Fragment and Child Measure services, adjacent vertical margin collapse, balanced Page/Fragment sink calls, stable derived IDs, pointer-independent 128-bit Plan Keys, and bounded structural validation. Virtual pages, columns, inline object layout, floats, overlays, keep/widow/orphan rules, and Playground projection remain pending and return `FW_STATUS_UNSUPPORTED`.
